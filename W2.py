...
welcome_text = ('На столе лежит 2021 конфета. Играют два игрока делая ход друг после друга. Первый ход определяется жеребьёвкой. За один ход можно забрать не более чем 28 конфет. Выигрывает тот, кто последним ходом заберет все конфеты.')
print(welcome_text)

message = ['Твоя очередь, ', 'Скорее, ', 'Твой ход, ', 'Бери быстрее, ']
...
def player_vs_player():
    ...
    print(f'\n{player_1} и {player_2} игра началась!\n')
    ...
    while candies_total > 0:
        if count == 0:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nЭй! {lucky} можно взять только {max_take} конфет, попробуй еще раз: '))
        ...
        if count == 1:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nЭй! {loser} можно взять только {max_take} конфет, попробуй еще раз: '))
            candies_total = candies_total - step
        if candies_total > 0:
            print(f'\nОсталось {candies_total} конфет')
            count = 0
    ...
    if count == 1:
        print(f'Игрок {loser} победил')
    if count == 0:
        print(f'Игрок {lucky} победил')
